...
void DumpPacket(DWORD packetType, DWORD connectionId, DWORD opcode, BYTE opcodeSize, DWORD size, PBYTE buffer)
{
    mtx.lock();
...
const ProtoEntry ProtoTable[] = {
    ...
    /* 7 */{ &SendHook, &RecvHook_WOD, "Legion"    },
};
...
DWORD MainThreadControl(LPVOID  param)
{
    ...
    printf("Welcome to WIPS, a WoW injector paket sniffer.\n");
    printf("WIPS is distributed under the GNU GPLv3 license.\n");
    printf("Source code is available at: ");
    printf("http://github.com/Konctantin/WIPS\n\n");
